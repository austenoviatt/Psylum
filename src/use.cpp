  ...
  if ( item == "poisoncoffee") {
    if (player->inventory.hasItem("poisoncoffee") == true) {
      if (result.size() == 2) {
        ...
        cout << "You look away in horror. Quickly the noises he is making subsides and when you look back, he is dead."
             << std::endl << endl;
    ...
    if (player->inventory.hasItem("sedative") == true) {
      ...
      if (result.size() == 2) {
        cout << "You inject yourself with the sedatives. You have always wondered what it feels like to be drugged. As you slowly lose your grip on consciousness, a doctor comes towards you and picks you up, and you finally slip into oblivion. When you come to a few hours later, you are back in your room, strapped to your bed. There is no way to get out now."
             << std::endl << endl;
        player->killPlayer();
      } else if ((player->currentRoom->characters[0].getName()== "patient")
                 && ((result[2] == "patient") || (result[2] == "annabelle")
                     || (result[2] == "girl") || result[2] == "person")) {
    ...
    if (player->inventory.hasItem("worm") == true) {
      if (result.size() == 2) {
        ...
        cout << "Keep the fishing rod pal, I don’t need it any more!’" << endl <<
             endl;
    ...
    if (player->inventory.hasItem("coffee") == true) {
      if (result.size() == 2) {
        cout << "You drink the coffee. It is warm and comforting, but you can't help feeling like you should have done something else with it."
             << endl << endl;
        ...
        cout << "You extend the cup of coffee to the doctor. He takes it impatiently and quickly downs the whole cup. You’re not sure how he didn’t burn himself, it was very hot. 'Another one!' he barks at you."
             << endl << endl;
        player->inventory.removeItem("coffee");
      } else if ((result[2] == "ratpoison")
                 && (player->inventory.hasItem("ratpoison") == true)) {
    ...
    if (player->currentRoom->inventory.hasItem("keurig")) {
      if (result.size() > 2) {
        std::cout << "You can't use the keurig like that." << endl << endl;
    ...
    if (player->inventory.hasItem("button2") == true) {
      ...
      std::cout << "Inventory does not contain this item." << std::endl << endl;
  } else if (item == "labcoat") {
    if (player->inventory.hasItem("labcoat") == true) {
      if (result.size() > 2) {
        std::cout << "You can't use the Lab Coat like that." << std::endl << endl;
      }
      else
        std::cout << "You put the Lab Coat on, you look very official. " << std::endl << std::endl;
      }
    ...
    if (player->inventory.hasItem("magicmushroom") == true) {
      ...
      if (player->currentRoom->getName() == "Ice Room") {
        for (uint i = 0; i < player->currentRoom->exits.size(); i++) {
          if (player->currentRoom->exits[i]->getKey() == "magicmushroom") {
            ...
            roomFound = true;
            std::cout <<
                      "You pop the mushrooms into your mouth and chew. They have an earthy taste, with a strange lingering heat, "
                      "like you just ate a habenero pepper. The mushrooms also seems to have another lingering effect, and the room "
                      "starts to swirl around you. You feel like you’re losing touch with reality and feel yourself transform into a dragon"
                      ", wings, scales, fire breath and all. As you explore the room and "
                      "your new dragon body, every fiery exhale seems to melt more and more of the ice. And then, as suddenly as it came on, "
                      "you are back in your own body, and the ice is all melted. You aren’t entirely sure what just happened. "
                      "You look into your pocket, and the magic mushroom is gone."
                      << endl << endl;
          }
        }
      }
      ...
      if (roomFound == false) {
        for (int i = 0; i < player->allInv.getItems().size(); i++) {
          if (player->allInv.items[i].getName() == "magicmushroom") {
            if (player->allInv.items[i].getItemState() == 0) {
              std::cout << "You take a nibble of the mushroom. They have an earthy taste, with a strange lingering heat,"
              " like you just ate a habenero pepper. You start feeling a little light headed." << std::endl << endl;
              player->allInv.items[i].increaseItemState();
            }
            else if (player->allInv.items[i].getItemState() == 1) {
              std::cout << "You take a nibble of the mushroom. They have an earthy taste, with a strange lingering heat,"
              " like you just ate a habenero pepper. You start getting a headache." << std::endl << endl;
              player->allInv.items[i].increaseItemState();
            }
            else if (player->allInv.items[i].getItemState() == 2) {
              std::cout << "You take a nibble of the mushroom. They have an earthy taste, with a strange lingering heat,"
              " like you just ate a habenero pepper. You got a sharp pain in your stomach and the headache has gotten worse." << std::endl << endl;
              player->allInv.items[i].increaseItemState();
            }
            else if (player->allInv.items[i].getItemState() == 3) {
              std::cout << "The stomach pain and headache are getting more and more intense. The room starts to swirl around you and the light is getting so bright it hurts to have your eyes open. You shut your eyes and curl up into a ball on the floor, and start vomit violently. Your whole body aches now and you can hardly breath from how often you are heaving. slowly the pain starts to fade, but a small part of you is cognizant of the fact that the pain isn't getting better, you are just becoming less aware of it. The lights start to fade along with the throbbing in your head. You slip into a blissful blackness. You are dead. "
              << std::endl << endl;
              player->killPlayer();
            }
          }
    ...
    for (int i = 0; i < player->inventory.getItems().size(); i++) {
      if (player->inventory.getItems()[i].getName() == "wolf"
          || player->inventory.getItems()[i].getName() == "goat"
          || player->inventory.getItems()[i].getName() == "cabbage") {
        numOfPets++;
      }
    }
    if (numOfPets > 1) {
      std::cout <<
                "You can only carry 1 pet onto the boat at a time, try dropping off a pet by using the \e[1mDrop\e[0m command."
                << std::endl;
      return;
    }

    //numOfPets ok, count the pets that's going to be left on the platform.
    if ((player->allInv.getItems()[wolfIndex].getItemState() ==
         player->allInv.getItems()[boatIndex].getItemState())
        && (player->inventory.hasItem("wolf") == false)) {
      std::cout << "You will be leaving \e[1mWolf\e[0m on the platform unattended" <<
                std::endl;
      //cout << "state of wolf is " << player->allInv.getItems()[wolfIndex].getItemState() << endl;
      pets.push_back("wolf");
    }
    if ((player->allInv.getItems()[goatIndex].getItemState() ==
         player->allInv.getItems()[boatIndex].getItemState())
        && (player->inventory.hasItem("goat") == false)) {
      std::cout << "You will be leaving \e[1mGoat\e[0m on the platform unattended" <<
                std::endl;
      //cout << "state of goat is " << player->allInv.getItems()[goatIndex].getItemState() << endl;
      pets.push_back("goat");
    }
    if ((player->allInv.getItems()[cabbageIndex].getItemState() ==
         player->allInv.getItems()[boatIndex].getItemState())
        && (player->inventory.hasItem("cabbage") == false)) {
      std::cout << "You will be leaving \e[1mCabbage\e[0m on the platform unattended"
                << std::endl;
    ...
    std::cout << std::endl;
    std::cout << "Everything ok and travel to the other side of the platform?" <<
              std::endl << endl;
    ...
    if (input == "1") {
      ...
      for (auto x : pets) {
        if (x == "wolf") {
          for (auto y : pets) {
            if (y == "goat") {
              std::cout <<
                        "As you made your way to the other side of the platform, due to being unattended, the \e[1mWolf\e[0m ate the \e[1mGoat\e[0m"
                        << std::endl << endl;
              std::cout <<
                        "The pet owner let out a horrifying scream, he couldn't handle the loss of his beloved \e[1mGoat\e[0m. He jumped into the dark, murky sewage water and drowned."
                        << std::endl << endl;
              std::cout <<
                        "You never figured out the password for the iron door and died in the sewage....."
                        << std::endl << endl;
              player->killPlayer();
              return;
            }
          ...
          for (auto y : pets) {
            if (y == "cabbage") {
              std::cout <<
                        "As you made your way to the other side of the platform, due to being unattended, the \e[1mGoat\e[0m ate the \e[1mCabbage\e[0m"
                        << std::endl << endl;
              std::cout <<
                        "The pet owner let out a horrifying scream, he couldn't handle the loss of his beloved \e[1mCabbage\e[0m. He jumped into the dark, murky sewage water and drowned."
                        << std::endl << endl;
              std::cout <<
                        "You never figured out the password for the iron door and died in the sewage....."
                        << std::endl << endl;
      ...
      if (player->allInv.getItems()[wolfIndex].getItemState() == 1
          && player->allInv.getItems()[goatIndex].getItemState() == 1
          && player->allInv.getItems()[cabbageIndex].getItemState() == 1) {
        std::cout <<
                  "You finally moved all 3 pets to the other side of the platform. The pet owner is sooooooo happy he swam across the sewage water and unlocked the \e[1miron\e[0m door for you."
                  << std::endl << endl;
        ...
        player->currentRoom->increaseRoomState();
        std::cout << "The \e[1miron\e[0m door is now open" << endl << endl;
        return;
      }
